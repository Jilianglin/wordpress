...
class Themify_Custom_Fonts {
	...
	public static function admin_footer() {
		...
		<div id="tf_font_import">
			...
			<select>
				...
				<option value="<?php echo esc_attr( $font['label'] ); ?>"><?php echo $font['label']; ?></option>
				<?php endforeach; ?>
			</select>
	...
	public static function download_google_css( $name ) {
		...
		$url = 'https://fonts.googleapis.com/css?family=' . urlencode( $fonts[ $name ]['label'] );
		if ( ! empty( $fonts[ $name ]['variants'] ) ) {
			$variants = [];
			foreach ( $fonts[ $name ]['variants'] as $variant ) {
				if ( $variant === 'regular' ) {
					$variant = '400';
