...
final class Themify_Builder_Stylesheet {
	...
	*/
	private  static function reSaveData($data){
		...
		unset($r['row_order']);
		$r['element_id'] = Themify_Builder_Model::checkUniqId($r['element_id']);
		if (!empty($r['cols'])) {
		    foreach($r['cols'] as &$c){
			...
			$c['element_id'] = Themify_Builder_Model::checkUniqId($c['element_id']);
			if(isset($c['cid'])){
			    unset($c['cid']);
			}
			if (!empty($c['modules'])) {
				...
				unset($m['row_order']);
				$m['element_id'] = Themify_Builder_Model::checkUniqId($m['element_id']);
				...
				if (!empty($m['cols'])) {
					...
					$sub_col['element_id'] = Themify_Builder_Model::checkUniqId($sub_col['element_id']);
					...
					if (!empty($sub_col['modules'])) {
						...
						$sub_m['element_id'] = Themify_Builder_Model::checkUniqId($sub_m['element_id']);
