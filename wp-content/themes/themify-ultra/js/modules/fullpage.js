...
(function ( Themify, document, window, themifyScript) {
	...
	const _is_retina = window.devicePixelRatio > 1,
		...
		_CLICK_ = !Themify.isTouch ? 'click' : (window.PointerEvent ? 'pointerdown' : 'touchstart'),
			...
			_wheel = function (e) {
				if(scrolling === false && _allowScrolling(e)===true){
				...
				if(scrolling===false){
					...
					if (row) {
						...
						if (silient !== true && row.style.transform) {
							...
							row.style.setProperty('transition','transform ' + duration + 'ms ease','important');
							row.style['transform'] = '';
						}
					}
					if(nav){
				...
				if (scrolling === false) {
					...
					if (item) {
						let index = parseInt(item.dataset['index']) || 0,
							isHorizontalScroll = isHorizontal === true || (item.classList.contains('fp-section-container-horizontal') ? (isNumber || snakeScroll) : false);
						...
						if(isHorizontalScroll && isNumber && (!horizontalChilds[horizontalIndex] || (horizontalChilds[horizontalIndex] && !item.contains(horizontalChilds[horizontalIndex])))){
